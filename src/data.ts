...
export function* entries<T, K extends keyof T>(obj: T | None, warn = true): Generator<[K, T[K]]> {
  if (obj instanceof Map) {
    return obj.entries();
  }
...
export function* values<T, K extends keyof T>(obj: T | None, warn = true): Generator<T[K]> {
  if (obj instanceof Map) {
    return obj.values();
  }
